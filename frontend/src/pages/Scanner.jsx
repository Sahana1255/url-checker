...
function Scanner() {
 ...
 const analyzeUrl = async (inputUrl) => {
  try {
    const res = await fetch('http://127.0.0.1:5000/analyze', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: inputUrl }) });
...
const transformBackendResponse = (backendData) => {
  ...
  const presentHeaders=[]; if(sh.strict_transport_security)presentHeaders.push("HSTS"); if(sh.content_security_policy)presentHeaders.push("CSP"); if(sh.x_content_type_options)presentHeaders.push("X-Content-Type-Options"); if(sh.x_frame_options)presentHeaders.push("X-Frame-Options"); if(sh.referrer_policy)presentHeaders.push("Referrer-Policy");
  ...
  return {url:backendData.url,riskScore:backendRisk,classification,pie:{series:[nSafe,nSusp,nDanger],labels:['Safe','Suspicious','Dangerous'],colors:['#344F1F','#FAB12F','#DD0303']},details:{sslValid:eSSL.https_ok||false,sslExpired:eSSL.expired||false,sslSelfSigned:eSSL.self_signed_hint||eSSL.self_signed||false,sslData:{...eSSL,certificate_valid:eSSL.certificate_valid,hostname_match:eSSL.hostname_match,serial_number:eSSL.serial_number,issuer_org:eSSL.issuer_org,subject_org:eSSL.subject_org,key_algorithm:eSSL.key_algorithm,key_size:eSSL.key_size,signature_algorithm:eSSL.signature_algorithm,san_domains:eSSL.san_domains||[],wildcard_cert:eSSL.wildcard_cert,chain_length:eSSL.chain_length,full_chain:eSSL.full_chain||[]},whoisAgeMonths,openPorts:[],securityHeaders:presentHeaders,keywords,mlPhishingScore,httpStatus:h.status||null,redirects:h.redirects||0,httpsRedirect:h.https_redirect,domainAge:whois.age_days||0,registrar:whois.registrar||"Unknown",whoisData:whois,headersData:h,errors:{ssl:eSSL.errors||[],headers:h.errors||[],whois:whois.errors||[],idn:idn.errors||[],rules:rules.errors||[]},scanTime:new Date().toISOString()}};
};
...
const onScan = async () => {
  ...
  try {
    const res = transformBackendResponse(await analyzeUrl(url.trim()));
 ...
 return (
  <div className="min-h-screen bg-white dark:bg-black flex flex-col items-center justify-center px-4">
    ...
    <div className="relative mb-10 flex items-center bg-gradient-to-r from-cyan-500/10 to-blue-500/10 dark:from-cyan-500/10 dark:to-blue-500/10 border border-cyan-500/40 dark:border-cyan-500/30 rounded-full px-20 py-5 backdrop-blur-sm">
      ...
      <input value={url} onChange={e=>setUrl(e.target.value)} placeholder="Enter URL to scan..." className="flex-1 text-lg bg-transparent text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-500 outline-none" onKeyPress={e=>e.key==='Enter'&&onScan()} disabled={loading} autoFocus />
      <button onClick={onScan} disabled={loading||!url.trim()} className="ml-4 px-8 py-3 bg-cyan-500 hover:bg-cyan-600 dark:bg-cyan-400 dark:hover:bg-cyan-300 disabled:bg-gray-400 dark:disabled:bg-gray-600 disabled:opacity-50 text-white dark:text-black font-semibold rounded-full transition-all duration-200 disabled:cursor-not-allowed">
        {loading ? <span className="flex items-center gap-2"><svg className="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>Scanning</span> : 'Scan'}
      </button>
    </div>

    <div className="flex items-center justify-center gap-10 mb-20">
      {['Quick Scan','Deep Analysis'].map((label,i)=>
        <button key={i} onClick={onScan} disabled={loading||!url.trim()} className="px-8 py-3 bg-gray-200 hover:bg-gray-300 dark:bg-black disabled:bg-gray-200 dark:disabled:bg-gray-900 disabled:opacity-50 text-gray-700 dark:text-gray-300 rounded-lg transition-all duration-200 border border-gray-400 dark:border-gray-700 disabled:cursor-not-allowed">{label}</button>
      )}
    </div>
    ...
    <div className="max-w-4xl mx-auto px-4 mb-12 flex flex-wrap justify-center gap-6">
      {[{name:"SSL/TLS Check",icon:<svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>},
        ...
        <div key={i} className="text-center px-4 py-2"><div className="text-gray-600 dark:text-gray-400 mb-1 flex justify-center">{f.icon}</div><div className="text-sm font-medium text-gray-700 dark:text-blue-300">{f.name}</div></div>
      )}
    </div>

    {error && <div className="mt-6 p-4 bg-red-100 dark:bg-red-500/10 border border-red-400 dark:border-red-500/30 rounded-lg backdrop-blur-sm">
