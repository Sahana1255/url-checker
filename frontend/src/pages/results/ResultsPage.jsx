...
import InteractivePieChart from "../../components/InteractivePieChart.jsx"; // Updated path
...
function ResultsPage({ result, onNewScan, expandedRows, setExpandedRows, showNewScanModal, setShowNewScanModal, confirmNewScan }) {
  ...
  const updatedResult = { ...result, pie: { ...result.pie, series: securityScores.pieData } };

  return (
    <div className="min-h-screen bg-white dark:bg-black">
      <NewScanModal isOpen={showNewScanModal} onClose={() => setShowNewScanModal(false)} onConfirm={confirmNewScan} />
      
      {/* Header */}
      <div className="border-b border-gray-300 dark:border-gray-700 bg-white dark:bg-black flex justify-between px-6 py-3">
        <button onClick={onNewScan} className="inline-flex items-center gap-2 text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 transition-colors duration-200 px-3 py-1.5 rounded-lg hover:bg-blue-50 dark:hover:bg-gray-700 text-sm border border-blue-300 dark:border-blue-600 hover:border-blue-400 dark:hover:border-blue-500">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
          </svg> 
          New Scan
        </button>
        <h1 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Enhanced Security Scan Results</h1>
        <button onClick={() => window.jsPDF && exportPdf(result, securityScores)} className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-3 py-1.5 text-sm text-white hover:bg-blue-700 transition-colors duration-200 dark:bg-blue-500 dark:hover:bg-blue-600 border border-blue-700 dark:border-blue-400">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="w-full px-6 py-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-1 bg-white dark:bg-gray-900 rounded-lg border border-gray-300 dark:border-gray-700 p-4 shadow-sm">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">{result.url}</h2>
          <div className="flex items-center space-x-4 mb-2">
            <span className="text-sm text-gray-600 dark:text-gray-400">Risk Score:</span>
            <span className={`text-2xl font-bold ${result.riskScore>=70?'text-red-600 dark:text-red-400':result.riskScore>=40?'text-yellow-600 dark:text-yellow-400':'text-green-600 dark:text-green-400'}`}>
              {result.riskScore}
            </span>
            <span className="text-sm text-gray-600 dark:text-gray-400">Total Score:</span>
            <span className="text-lg font-semibold text-gray-900 dark:text-gray-100">{securityScores.overall}%</span>
          </div>
          <div className="flex items-center space-x-4 mb-4">
            <span className="text-sm text-gray-600 dark:text-gray-400">Classification:</span>
            <span className={`text-lg font-semibold ${result.classification==='High Risk'?'text-red-600 dark:text-red-400':result.classification==='Medium Risk'?'text-yellow-600 dark:text-yellow-400':'text-green-600 dark:text-green-400'}`}>
              {result.classification}
            </span>
          </div>
          <div className="border-t border-gray-300 dark:border-gray-700 pt-3">
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-2">Risk Analysis</h3>
            <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
              Overall risk score is {result.riskScore} classifying as {result.classification}. {result.details.keywords?.length>0?`Detected risky keywords: ${result.details.keywords.join(', ')}.`:'No suspicious keywords detected.'}
            </p>
          </div>
        </div>
        
        <div className="lg:col-span-2 bg-white dark:bg-gray-900 rounded-lg border border-gray-300 dark:border-gray-700 p-4 shadow-sm">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">Risk Composition</h3>
...
const exportPdf = (result, securityScores) => {
  try {
    ...
    const comp = ["Safe", "Suspicious", "Dangerous"].map((l, i) => `${l}: ${securityScores.pieData?.[i] || ""}%`).join(" |");
